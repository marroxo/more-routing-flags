...
namespace MoreRoutingFlags {
    ...
    internal class FlyCamera {
        ...
        private List<Renderer> hiddenClutter;
        private Dictionary<Terrain, float> hiddenTerrainTreeDistances;

        /**
         * <summary>
         * Hides (or restores) only the trees actually blocking a
         * clear view of a flag from the current camera position --
         * not every tree in the scene. Not layer-based -- confirmed
         * via in-game diagnostic that every tree renderer is on layer
         * 0 (Default), same as everything else, so there's no
         * dedicated layer to cull. Finds candidates by name and
         * disables the ones whose bounds intersect the line of sight
         * to at least one flag.
         * </summary>
         * <param name="hidden">Whether clutter should be hidden</param>
         * <param name="flags">The flags to check line of sight to (ignored when hidden is false)</param>
         */
        internal static void SetClutterHidden(bool hidden, List<Flag> flags = null) {
            if (instance == null) {
                return;
            }

            if (hidden == false) {
                if (instance.hiddenClutter != null) {
                    foreach (Renderer renderer in instance.hiddenClutter) {
                        if (renderer != null) {
                            renderer.gameObject.SetActive(true);
                        }
                    }

                    instance.hiddenClutter = null;
                }
            ...
            instance.hiddenClutter = new List<Renderer>();
            ...
            Vector3 camPos = instance.root.transform.position;

            List<(Vector3 target, float distance)> sightLines = new List<(Vector3, float)>();
            foreach (Flag flag in flags) {
                Vector3 target = FlagPlacer.Resolve(flag);
                sightLines.Add((target, Vector3.Distance(camPos, target)));
            }

            foreach (Renderer renderer in GameObject.FindObjectsOfType<Renderer>()) {
                string name = renderer.gameObject.name.ToLowerInvariant();

                // Confirmed via scene dump: close trees + distant billboard impostor
                bool isTree = name.Contains("treeline") == true
                    || name.Contains("conifer") == true;

                if (isTree == false) {
                    continue;
                }

                // TEMPORARY: sight-line check disabled below to isolate the hide mechanism
                bool inTheWay = true;
                /*
                foreach ((Vector3 target, float distance) in sightLines) {
                    Ray sightRay = new Ray(camPos, (target - camPos).normalized);
                    float hitDistance;

                    if (renderer.bounds.IntersectRay(sightRay, out hitDistance) == true
                ...
                */
                ...
                instance.hiddenClutter.Add(renderer);
            }

            // Renderer.enabled/SetActive had no effect -- these are likely Terrain-painted trees
            foreach (Terrain terrain in Terrain.activeTerrains) {
                instance.hiddenTerrainTreeDistances[terrain] = terrain.treeDistance;
                terrain.treeDistance = 0f;
            }

            logger.LogInfo($"SetClutterHidden(true): found {Terrain.activeTerrains.Length} terrain(s), zeroed treeDistance");

            logger.LogInfo($"SetClutterHidden(true): {instance.hiddenClutter.Count} trees in the way of {flags.Count} flags, cam={camPos}");
        }
        ...
        private IEnumerator FlightRoutine(Flag flag, Action onArrive) {
            try {
                ...
                yield return Ease(startPos, endPos, startRot, endRot, Config.flyDuration.Value);
            }
            finally {
                flight = null;
                onArrive?.Invoke();
         ...
         * <param name="flags">The flags to frame</param>
         */
        internal static void FrameAll(List<Flag> flags) {
